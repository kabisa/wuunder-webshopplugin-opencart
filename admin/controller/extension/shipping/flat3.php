...
class ControllerExtensionShippingFlat3 extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/shipping/flat3', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/shipping/flat3', 'token=' . $this->session->data['token'], true);
        ...
        if (isset($this->request->post['flat3_wuunder_filter'])) {
            $data['flat3_wuunder_filter'] = $this->request->post['flat3_wuunder_filter'];
        } else {
            $data['flat3_wuunder_filter'] = $this->config->get('flat3_wuunder_filter');
        }
